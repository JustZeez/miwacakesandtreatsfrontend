...
import { Search, Package, Clock, CheckCircle, Truck, Loader, AlertCircle } from 'lucide-react';
...
const TrackOrder = () => {
  ...
  const handleTrackOrder = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.message || 'Order not found. Please check your details.');
      setOrder(null);
    } finally {
      setLoading(false);
    }
        ...
        <div className="max-w-2xl mx-auto px-4 text-center">
          <div className="bg-white rounded-3xl p-8 shadow-xl border border-emerald-200">
            ...
            <p className="text-gray-600 mb-8">
              Your order has been delivered and is no longer available for tracking.
              Thank you for choosing Miwa Cakes!
            </p>
            <button
              onClick={() => {
                setOrder(null);
        ...
        {order && (
          <div className="bg-white rounded-3xl p-8 shadow-xl border border-pink-100">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className={`px-4 py-2 rounded-full font-bold ${statusConfig[order.status]?.color}`}>
            ...
            {/* Status Timeline */}
            <div className="mb-10">
              <h3 className="font-bold text-lg text-gray-900 mb-6">Order Progress</h3>
              <div className="relative">
                {/* Timeline Line */}
                <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-gray-200"></div>
                
                {/* Status Steps */}
                {['pending', 'confirmed', 'preparing', 'on delivery', 'delivered'].map((status, index) => {
                  const isCompleted = getStatusIndex(order.status) >= index;
                  const isCurrent = order.status === status;
                  
                  return (
                    <div key={status} className="relative flex items-start mb-8">
                      <div className={`z-10 w-16 h-16 rounded-full flex items-center justify-center ${isCompleted ? 'bg-gradient-to-r from-pink-500 to-purple-500' : 'bg-gray-200'}`}>
                        {isCompleted ? statusConfig[status]?.icon : <div className="w-6 h-6"></div>}
                      </div>
                      <div className="ml-6 flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h4 className={`font-bold text-lg ${isCompleted ? 'text-gray-900' : 'text-gray-400'}`}>
              ...
              <div className="bg-gray-50 p-6 rounded-2xl">
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-3 border-t border-gray-300 flex justify-between">
                    ...
                    <span className="text-xl font-bold text-pink-600">₦{order.totalAmount?.toLocaleString()}</span>
                  </div>
                </div>
            ...
            <div className="mt-8 pt-8 border-t border-gray-200 text-center">
              <p className="text-gray-600">
                Need help? Contact us on WhatsApp: <span className="font-bold text-pink-600">+234 123 456 7890</span>
              </p>
            </div>
          </div>
        )}
...
const getStatusIndex = (status) => {
  const statusOrder = ['pending', 'confirmed', 'preparing', 'on delivery', 'delivered'];
