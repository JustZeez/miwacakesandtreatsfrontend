...
import { 
  Sparkles, Cake, Package, CheckCircle, Clock, 
  Truck, AlertCircle, TrendingUp, DollarSign, 
  ShoppingBag, Bell, User, LogOut, Eye, Edit, 
  Trash2, Filter, Download, MoreVertical, Search,
  ChevronLeft, ChevronRight, RefreshCw, Loader,
...
const fetchDashboardData = async () => {
  try {
    ...
    // Fetch BOTH stats AND all orders
    const [statsResponse, ordersResponse] = await Promise.all([
      adminAPI.getDashboardStats(),
      adminAPI.getAllOrders() // This gets ALL orders
    ]);
    ...
    // Update orders from API - IMPORTANT: Map ALL fields correctly
    if (ordersResponse.data) {
      setOrders(ordersResponse.data.map(order => ({
        id: order.orderId || order._id, // Use orderId if available
        customer: order.customerName, // Changed from customer to customerName
        amount: `₦${order.totalAmount?.toLocaleString() || '0'}`, // Use totalAmount
        items: order.cartItems?.length || 0, // Count cart items
        status: order.status,
        ...
        avatar: order.customerName?.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2) || 'CU',
        color: getStatusColor(order.status),
        phone: order.phone || 'N/A',
        address: order.address || 'N/A',
        paymentProof: order.paymentProofUrl || order.paymentProof, // Use paymentProofUrl from backend
        _id: order._id,
 ...
 const handleUpdateStatus = async (orderId, newStatus) => {
  try {
    ...
    // You can add toast here: toast.success('Status updated successfully!');
  ...
  // Delete order
 // AdminDashboard.jsx - Update handleDeleteOrder
const handleDeleteOrder = async (orderId) => {
  if (!window.confirm('Are you sure you want to delete this order?')) {
    return;
  }

  // Get the row element for visual feedback
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6">
          ...
          <header className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Miwa Dashboard</h1>
              <p className="text-gray-500">Manage orders and track your bakery</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
            {stats.map((stat, index) => (
              <div key={index} className="bg-white rounded-xl shadow border border-gray-100 p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className={`p-3 rounded-lg ${stat.color} text-white shadow-sm`}>
                    {stat.icon}
                  </div>
                  <div className={`text-sm font-semibold ${stat.trend === 'up' ? 'text-emerald-600' : 'text-amber-600'}`}>
                    {stat.change}
                  </div>
                </div>
            ...
            <div className="lg:col-span-2 bg-white rounded-xl shadow border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-bold text-gray-900">
                    Recent Orders {filteredOrders.length > 0 && `(${filteredOrders.length})`}
                  </h3>
              ...
              {filteredOrders.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h4 className="text-lg font-medium text-gray-700">No orders found</h4>
                  ...
                  <table className="w-full min-w-[700px]">
                    ...
                    <tbody>
                      {filteredOrders.map((order, index) => (
                        <tr key={index} className="border-b border-gray-50 hover:bg-pink-50/30 transition-colors">
                          <td className="py-4 px-4">
                            <span className="font-mono text-sm font-bold text-pink-600">{order.id}</span>
                          </td>
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${order.color}`}>
                                {order.avatar}
                              </div>
                              <div>
                          ...
                          <td className="py-4 px-4">
                            <select 
                              ...
                              className={`text-xs font-bold px-3 py-1.5 rounded-full border-0 focus:ring-2 focus:ring-pink-200 focus:outline-none ${
                                ...
                                order.status === 'on delivery' ? 'bg-blue-100 text-blue-700' :
